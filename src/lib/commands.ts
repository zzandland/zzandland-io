import React from 'react'; // Needed for JSX types in processCommand
...
export const processCommand = (
  ...
  initialMessages: React.ReactNode[], // Still needed for 'clear'
): CommandResult => {
  // Start with the command echo
  let newOutputMessages: OutputMessage[] = [{ type: 'command', text: `> ${commandInput}` }];
  ...
  switch (command) {
      case Command.Help:
          // Return normal text
          newOutputMessages.push({ type: 'normal', text: `Available commands: ${availableCommands}` });
          break;
      case Command.Clear:
          shouldClear = true;
      ...
      case Command.Ls:
          ...
          if (showHidden) {
              filesToList = [...files.hidden, ...files.visible].sort((a, b) => a.name.localeCompare(b.name));
          } else {
      ...
      case Command.Open:
          ...
          if (!fileNameToOpen) {
              // Return warning text
              newOutputMessages.push({ type: 'warning', text: 'Usage: open <filename>' });
          } else if (fileNameToOpen === 'resume.pdf') {
              const resumeExists = [...files.visible, ...files.hidden].some(f => f.name === 'resume.pdf');
              if (resumeExists) {
                   // Return normal text
                   newOutputMessages.push({ type: 'normal', text: 'Opening resume.pdf...' });
                   action = { type: 'openModal', url: '/Si Yong Kim - Software Engineer.pdf' };
              } else {
                   // Return error text
                   newOutputMessages.push({ type: 'error', text: 'Error: resume.pdf definition not found.' });
              }
          } else {
              const fileExists = [...files.visible, ...files.hidden].some(f => f.name === fileNameToOpen);
              if (fileExists) {
                   // Return warning text
                  newOutputMessages.push({ type: 'warning', text: `Cannot open this file type yet: ${fileNameToOpen}` });
              } else {
                   // Return warning text
                  newOutputMessages.push({ type: 'warning', text: `File not found: ${fileNameToOpen}` });
              }
          }
          break;
      case Command.Unknown:
          if (commandInput !== '') {
              if (commandInput.startsWith('./')) {
                  // Return warning text
                  newOutputMessages.push({ type: 'warning', text: "Direct execution with './' is not supported. Use 'open <filename>'." });
              } else {
                  // Return error text and help text
                  newOutputMessages.push({ type: 'error', text: `Invalid command: '${commandInput}'` });
                  newOutputMessages.push({ type: 'normal', text: `Available commands: ${availableCommands}` });
              }
          }
          break;
