...
function findWasmBinary() {
  return locateFile("SDL2-sort.wasm");
}
...
async function createWasm() {
  function receiveInstance(instance, module) {
    ...
    registerTLSInit(wasmExports["Td"]);
    wasmTable = wasmExports["Od"];
    wasmModule = module;
...
function _eglCreateContext(display, config, hmm, contextAttribs) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(12, 0, 1, display, config, hmm, contextAttribs);
  if (display != 62e3) {
    EGL.setErrorCode(12296);
    return 0;
  }
...
function _eglCreateWindowSurface(display, config, win, attrib_list) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(13, 0, 1, display, config, win, attrib_list);
  if (display != 62e3) {
    EGL.setErrorCode(12296);
    return 0;
  }
...
function _eglDestroyContext(display, context) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(14, 0, 1, display, context);
  if (display != 62e3) {
    EGL.setErrorCode(12296);
    return 0;
  }
...
function _eglDestroySurface(display, surface) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(15, 0, 1, display, surface);
  if (display != 62e3) {
    EGL.setErrorCode(12296);
    return 0;
  }
...
function _eglGetConfigAttrib(display, config, attribute, value) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(16, 0, 1, display, config, attribute, value);
  if (display != 62e3) {
    EGL.setErrorCode(12296);
    return 0;
  }
...
function _eglGetDisplay(nativeDisplayType) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(17, 0, 1, nativeDisplayType);
  EGL.setErrorCode(12288);
...
function _eglInitialize(display, majorVersion, minorVersion) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(19, 0, 1, display, majorVersion, minorVersion);
  if (display != 62e3) {
    EGL.setErrorCode(12296);
    return 0;
  }
...
function _eglMakeCurrent(display, draw, read, context) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(20, 0, 1, display, draw, read, context);
  if (display != 62e3) {
    EGL.setErrorCode(12296);
    return 0;
  }
...
function _eglSwapInterval(display, interval) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(23, 0, 1, display, interval);
  if (display != 62e3) {
    EGL.setErrorCode(12296);
    return 0;
  }
...
function _eglWaitNative(nativeEngineId) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(26, 0, 1, nativeEngineId);
  EGL.setErrorCode(12288);
  return 1;
}
      ...
      if (currentFullscreenStrategy.canvasResizedCallback) {
        if (currentFullscreenStrategy.canvasResizedCallbackTargetThread)
          __emscripten_run_callback_on_thread(
            currentFullscreenStrategy.canvasResizedCallbackTargetThread,
...
var JSEvents_requestFullscreen = (target, strategy) => {
  ...
  if (strategy.canvasResizedCallback) {
    if (strategy.canvasResizedCallbackTargetThread)
      __emscripten_run_callback_on_thread(
        strategy.canvasResizedCallbackTargetThread,
...
function _emscripten_get_element_css_size(target, width, height) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(33, 0, 1, target, width, height);
...
function _emscripten_get_gamepad_status(index, gamepadState) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(34, 0, 1, index, gamepadState);
...
function _emscripten_request_pointerlock(target, deferUntilInEventHandler) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(38, 0, 1, target, deferUntilInEventHandler);
  target = findEventTarget(target);
  if (!target) return -4;
  if (!target.requestPointerLock) {
    return -1;
...
function _emscripten_set_beforeunload_callback_on_thread(
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(40, 0, 1, userData, callbackfunc, targetThread);
...
var registerFullscreenChangeEventCallback = (
  ...
  var fullscreenChangeEventhandlerFunc = (e = event) => {
    var fullscreenChangeEvent = targetThread
      ? _malloc(276)
...
var registerPointerlockChangeEventCallback = (
  ...
  var pointerlockChangeEventHandlerFunc = (e = event) => {
    var pointerlockChangeEvent = targetThread
      ? _malloc(257)
...
var registerVisibilityChangeEventCallback = (
  ...
  var visibilityChangeEventHandlerFunc = (e = event) => {
    var visibilityChangeEvent = targetThread
      ? _malloc(8)
...
function _environ_get(__environ, environ_buf) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(64, 0, 1, __environ, environ_buf);
  var bufSize = 0;
  var envp = 0;
...
function _environ_sizes_get(penviron_count, penviron_buf_size) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(65, 0, 1, penviron_count, penviron_buf_size);
...
function _fd_read(fd, iov, iovcnt, pnum) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(67, 0, 1, fd, iov, iovcnt, pnum);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_seek(fd, offset, whence, newOffset) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(68, 0, 1, fd, offset, whence, newOffset);
...
function _fd_write(fd, iov, iovcnt, pnum) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(69, 0, 1, fd, iov, iovcnt, pnum);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
...
var ASM_CONSTS = {
  ...
  92738: () => window.innerWidth,
  92768: () => window.innerHeight,
};
var wasmImports;
function assignWasmImports() {
  wasmImports = {
    ...
    Za: __abort_js,
    eb: __emscripten_init_main_thread_js,
    Ta: __emscripten_notify_mailbox_postmessage,
    ab: __emscripten_receive_on_main_thread_js,
    Ya: __emscripten_runtime_keepalive_clear,
    I: __emscripten_thread_cleanup,
    db: __emscripten_thread_mailbox_await,
    mb: __emscripten_thread_set_strongref,
    Ra: __tzset_js,
    Zb: _eglBindAPI,
    Oc: _eglChooseConfig,
    ...
    Ua: _eglDestroySurface,
    Zc: _eglGetConfigAttrib,
    P: _eglGetDisplay,
    Ma: _eglGetError,
    sc: _eglInitialize,
    bb: _eglMakeCurrent,
    ...
    E: _emscripten_asm_const_ptr_sync_on_main_thread,
    ...
    ka: _emscripten_get_num_gamepads,
    Ja: _emscripten_get_screen_size,
    ja: _emscripten_glActiveTexture,
    ia: _emscripten_glAttachShader,
    Aa: _emscripten_glBeginQueryEXT,
    ha: _emscripten_glBindAttribLocation,
    ga: _emscripten_glBindBuffer,
    fa: _emscripten_glBindFramebuffer,
    ea: _emscripten_glBindRenderbuffer,
    da: _emscripten_glBindTexture,
    sa: _emscripten_glBindVertexArrayOES,
    ca: _emscripten_glBlendColor,
    ...
    T: _emscripten_glClearStencil,
    ob: _emscripten_glClipControlEXT,
    S: _emscripten_glColorMask,
    ...
    Cc: _emscripten_glHint,
    Bc: _emscripten_glIsBuffer,
    Ac: _emscripten_glIsEnabled,
    ...
    y: _emscripten_set_blur_callback_on_thread,
    ...
    C: _emscripten_set_mouseenter_callback_on_thread,
    B: _emscripten_set_mouseleave_callback_on_thread,
...
var wasmExports;
createWasm();
var ___wasm_call_ctors = () => (___wasm_call_ctors = wasmExports["Nd"])();
var _main = (Module["_main"] = (a0, a1) =>
  (_main = Module["_main"] = wasmExports["Pd"])(a0, a1));
var _free = (a0) => (_free = wasmExports["Qd"])(a0);
var _malloc = (a0) => (_malloc = wasmExports["Rd"])(a0);
var _pthread_self = () => (_pthread_self = wasmExports["Sd"])();
var __emscripten_tls_init = () => (__emscripten_tls_init = wasmExports["Td"])();
var __emscripten_run_callback_on_thread = (a0, a1, a2, a3, a4) =>
  (__emscripten_run_callback_on_thread = wasmExports["Ud"])(a0, a1, a2, a3, a4);
var __emscripten_thread_init = (a0, a1, a2, a3, a4, a5) =>
  (__emscripten_thread_init = wasmExports["Vd"])(a0, a1, a2, a3, a4, a5);
var __emscripten_thread_crashed = () =>
  (__emscripten_thread_crashed = wasmExports["Wd"])();
var __emscripten_run_on_main_thread_js = (a0, a1, a2, a3, a4) =>
  (__emscripten_run_on_main_thread_js = wasmExports["Xd"])(a0, a1, a2, a3, a4);
var __emscripten_thread_free_data = (a0) =>
  (__emscripten_thread_free_data = wasmExports["Yd"])(a0);
var __emscripten_thread_exit = (a0) =>
  (__emscripten_thread_exit = wasmExports["Zd"])(a0);
var __emscripten_check_mailbox = () =>
  (__emscripten_check_mailbox = wasmExports["_d"])();
var _emscripten_stack_set_limits = (a0, a1) =>
  (_emscripten_stack_set_limits = wasmExports["$d"])(a0, a1);
var __emscripten_stack_restore = (a0) =>
  (__emscripten_stack_restore = wasmExports["ae"])(a0);
var __emscripten_stack_alloc = (a0) =>
  (__emscripten_stack_alloc = wasmExports["be"])(a0);
